...
//  Define what should happen when "bellRings" event occurs
myEmitter.on('bellRings', (username='Admin') => {
  console.log('Someone rang the bell! Opening the door...'+username);
});

// Trigger (emit) the event
myEmitter.emit('bellRings');
// Trigger (emit) the event again
myEmitter.emit('bellRings');
myEmitter.emit('bellRings', "Srinivas");
...
downloader.on('end', () => console.log('Download complete!'));
